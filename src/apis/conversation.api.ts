...
export const getUnreadConversationCount = () =>
  http.get<SuccessResponse<number>>('conversation/unread-count')

export const markMessageAsRead = (messageId: string) =>
  http.put<SuccessResponse<any>>(`message/${messageId}/read`)

export const markConversationAsRead = (conversationId: string) =>
  http.put<SuccessResponse<{ markedCount: number }>>(`conversation/${conversationId}/read`)
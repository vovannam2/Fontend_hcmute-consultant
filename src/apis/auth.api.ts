...
export const registerRequest = (body: { fullName: string; email: string; password: string; role: string; studentCode: string; phone: string; department?: string }) =>
  http.post<SuccessResponse<{ message: string; ttlMinutes: number }>>('auth/register/request', body)

export const registerVerify = (body: { email: string; code: string }) =>
  http.post<SuccessResponse<{ message: string; userId: string; email: string; studentCode: string }>>('auth/register/verify', body)

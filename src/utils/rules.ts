...
export const RegisterSchema = yup.object({
  username: yup.string().required('Bạn phải nhập họ và tên').min(5, 'Bạn phải nhập tối thiểu 5 ký tự'),
  email: yup.string().required('Bạn phải nhập email').email('Email không đúng định dạng'),
  password: yup.string().required('Bạn phải nhập mật khẩu').min(6, 'Mật khẩu phải có tối thiểu 6 ký tự'),
  confirmPassword: yup
    .string()
...
export const CreateQuestionSchema = yup.object({
  ...
  firstName: yup.string().required('Bạn phải nhập họ và tên đệm'),

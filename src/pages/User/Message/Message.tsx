...
export default function Message() {
  ...
  const { data: conversations } = useQuery({
    ...
    queryFn: async () => {
      const response = await getConversations(conversationQueryParams)
  ...
  useEffect(() => {
    if (!conversations?.data?.content || id) {
      setConversationActive(undefined)
      return
    }
    const data = conversations.data?.content
    if (data?.length !== 0) {
      navigate({
  ...
  useEffect(() => {
    if (!conversations?.data?.content || !id) return
    const data = conversations.data?.content
    const conversationActive = data.find((obj: Conversation) => obj.id === parseInt(id))
      ...
      <div className='grid grid-cols-12'>
        <div className='hidden lg:flex col-span-4 border-r flex-col h-remain-screen'>
          ...
          <div className='mt-3 flex-grow overflow-y-auto h-full'>
            {conversations?.data?.content.map((conversation: Conversation) => (
              <MessageItem
                key={conversation.id}

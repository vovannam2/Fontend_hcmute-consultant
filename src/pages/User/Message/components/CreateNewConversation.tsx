...
import { createGroupConversation, createUserConversation } from '@/apis/conversation.api.mock'
...
export default function CreateNewConversation({ conversationQueryParams }: Props) {
  ...
  const createConversationMutation = useMutation({
    mutationFn: async (body: ConversationFormData) => {
      const response = await createUserConversation(body)
  ...
  const createGroupConversationMutation = useMutation({
    mutationFn: async (body: GroupConversationFormData) => {
      const response = await createGroupConversation(body)
      // Kiểm tra nếu response có thuộc tính data và status (SuccessResponse)
      if ('data' in response && 'status' in response && 'message' in response) {
        return response
      }
